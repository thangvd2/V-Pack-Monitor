...
- 👤 **User Management & Audit Log:** CRUD user, session management, nhật ký hệ thống. Audit log đầy đủ (login, CRUD user/station, settings). Tự dọn sau 90 ngày.
- 🏥 **System Health Dashboard:** Giám sát CPU/RAM/Disk, FFmpeg processes, camera reachability. Auto-refresh 5s.
- 📡 **MediaMTX Auto-Install:** Installer tự động tải MediaMTX cho Live View. Fallback UI khi MediaMTX chưa chạy.
...
**Q: Camera trên dashboard thỉnh thoảng nhấp nháy khung viền đỏ?**
A: Đây là tính năng giám sát Health check. Viền đỏ có nghĩa là Camera đang bị Offline (mất nguồn, mất mạng) hoặc không thể truy cập. Hệ thống sẽ tự động nhắn tin cảnh báo Telegram nếu offline quá 5 phút.

**Q: Đang đóng hàng tự nhiên thấy video dừng và tự động lưu?**
...
**Q: Các port nào cần mở ở tường lửa (Firewall)?**
A: Port 554 (RTSP), 8889 (WebRTC), 8001 (FastAPI).

**Q: Trình duyệt nào được khuyên dùng?**
A: Hệ thống tối ưu tốt nhất cho **Google Chrome** và **Microsoft Edge**. (Safari không hỗ trợ đầy đủ WebRTC).

...
# Hướng Dẫn Sử Dụng V-Pack Monitor v2.1.0 (Dành Cho Quản Trị Viên)
...
Thanh Header nằm ở phía trên cùng màn hình, chứa các điều khiển chính (từ trái sang phải):

| Phần tử | Mô tả |
|----------|--------|
| **Logo + tiêu đề "V-Pack Monitor"** | Tên ứng dụng, luôn hiển thị bên trái |
| **Trạm:** (dropdown) | Chọn trạm hiện tại để làm việc. Danh sách trạm do ADMIN tạo |
| **[+]** (nút thêm) | Thêm trạm mới. Nhấn vào sẽ mở cửa sổ SetupModal |
| **LayoutGrid / Maximize2** | Chuyển chế độ xem: Single (đơn lẻ) hoặc Grid (lưới tổng quan). Nút này chỉ hiện khi hệ thống có từ 2 trạm trở lên |
| **BarChart3** | Mở Dashboard thống kê. Biểu tượng được highlight xanh khi đang bật |
| **Activity** | Mở Sức khỏe hệ thống (System Health). Biểu tượng highlight vàng khi đang bật. **Chỉ dành cho ADMIN** |
...
| **Settings** ("Cài đặt Camera & Hệ thống cho Trạm này") | Mở SetupModal để cấu hình trạm đang chọn. **Chỉ dành cho ADMIN** |
| **Users** ("Quản lý người dùng") | Mở UserManagementModal. **Chỉ dành cho ADMIN** |
...
Vùng nội dung chính hiển thị một trong bốn chế độ sau:

1. **Camera Live + Lịch sử ghi hình** (mặc định khi mở ứng dụng). Hiển thị camera trực tiếp ở bên trái và danh sách lịch sử ghi hình ở cột bên phải.
2. **Dashboard thống kê**. Nhấn nút **BarChart3** trên thanh Header để mở. Xem chi tiết tại [Mục 8](#8-dashboard--thống-kê).
3. **System Health (Sức khỏe hệ thống)**. Nhấn nút **Activity** trên thanh Header. Chỉ ADMIN mới thấy nút này. Xem chi tiết tại [Mục 9](#9-sức-khỏe-hệ-thống-chỉ-admin).
4. **Grid Tổng quan toàn kho**. Nhấn nút **LayoutGrid** trên thanh Header khi hệ thống có từ 2 trạm trở lên. Xem chi tiết tại [Mục 7](#7-grid-tổng-quan-toàn-kho).
...
| **Chế độ ghi Video** | Chọn cách ghi video cho trạm. Các chế độ可用: |
...
### 9.4 Trạng Thái Camera

Bảng kiểm tra kết nối tới từng camera:

| Trạng thái | Biểu tượng | Ý nghĩa |
|-------------|------------|----------|
| Kết nối | 🟢 | Camera phản hồi ping bình thường |
| Mất kết nối | 🔴 | Camera không phản hồi. Kiểm tra dây mạng, nguồn điện, và IP camera |
...
4. Lưu cấu hình.
5. Quay lại giao diện chính, nhấn **CloudUpload** để bắt đầu đẩy video lên cloud.

...
def _read_version():
    try:
        vpath = os.path.join(os.path.dirname(os.path.dirname(__file__)) or ".", "VERSION")
...
def _mtx_add_path(station_id, rtsp_url, suffix="", station_name=""):
    ...
    try:
        ...
        _logger.info(f"[MTX] Registered path {name}{tag}")
        return
    except Exception as e:
        _logger.error(f"[MTX] replace {name} failed: {e}")
    try:
        req = urllib.request.Request(
    ...
    try:
        ...
        _logger.error(f"[MTX] ERROR: add {name} failed after all retries: {e}")
...
def _mtx_remove_path(station_id, suffix="", station_name=""):
    ...
    try:
        ...
        urllib.request.urlopen(req, timeout=5)
        tag = f' "{station_name}"' if station_name else ""
        _logger.info(f"[MTX] Removed path {name}{tag}")
    except urllib.error.HTTPError as e:
        if e.code != 404:
            _logger.error(f"[MTX] delete {name} failed: {e}")
    except Exception as e:
        _logger.warning(f"[MTX] Failed to remove path {name}: {e}")
    ...
    def _monitor_loop(self):
        while self.is_running:
            ...
            try:
                ...
                tag = f' "{self.station_name}"' if self.station_name else ""
                _logger.error(f"[MTX] monitor loop error for station_{self.station_id}{tag}: {e}")

...
class TestLogin:
    def test_login_success(self, client):
        database.update_user_password(
            database.get_user_by_username("admin")["id"], "AdminPass1!"
        )
...
class TestAuthEndpoints:
    ...
    def test_change_password_too_short(self, client, admin_headers):
        r = client.put("/api/auth/change-password", headers=admin_headers, json={
            "old_password": "whatever", "new_password": "short"
        })
...
class TestStationCRUD:
    def test_create_station(self, client, admin_headers):
        r = client.post("/api/stations", headers=admin_headers, json={
            "name": "New Station", "ip_camera_1": "10.0.0.1", "ip_camera_2": "",
            "safety_code": "ABC123", "camera_mode": "SINGLE"
        })
        assert r.status_code == 200
        assert r.json()["status"] == "success"
        assert "id" in r.json()
    ...
    def test_update_station(self, client, admin_headers, sample_station_id):
        r = client.put(f"/api/stations/{sample_station_id}", headers=admin_headers, json={
            "name": "Updated", "ip_camera_1": "10.0.0.99", "ip_camera_2": "",
            "safety_code": "NEWCODE", "camera_mode": "DUAL_FILE", "camera_brand": "dahua"
        })
        assert r.json()["status"] == "success"
    ...
    def test_check_conflict_ip(self, client, admin_headers, sample_station_id):
        r = client.get("/api/stations/check-conflict", headers=admin_headers, params={"ip": "192.168.5.18"})
        assert len(r.json()["warnings"]) > 0

    def test_check_conflict_mac(self, client, admin_headers, sample_station_id):
        r = client.get("/api/stations/check-conflict", headers=admin_headers, params={"mac": "AA:BB:CC:DD:EE:FF"})
        assert len(r.json()["warnings"]) > 0

    def test_check_conflict_name(self, client, admin_headers, sample_station_id):
        r = client.get("/api/stations/check-conflict", headers=admin_headers, params={"name": "Test Station"})
        assert len(r.json()["warnings"]) > 0

    def test_check_conflict_exclude_self(self, client, admin_headers, sample_station_id):
        r = client.get("/api/stations/check-conflict", headers=admin_headers,
...
class TestRecords:
    ...
    def test_get_status_idle(self, client, admin_headers, sample_station_id):
        r = client.get("/api/status", headers=admin_headers, params={"station_id": sample_station_id})
...
class TestSettings:
    ...
    def test_update_settings(self, client, admin_headers):
        r = client.post("/api/settings", headers=admin_headers, json={
            "RECORD_KEEP_DAYS": 30, "CLOUD_PROVIDER": "NONE"
        })
        assert r.json()["status"] == "success"
    ...
    def test_update_settings_preserve_masked(self, client, admin_headers):
        ...
        r = client.post("/api/settings", headers=admin_headers, json={
            "RECORD_KEEP_DAYS": 7, "S3_SECRET_KEY": "****"
        })
...
class TestSessions:
    def test_acquire_session(self, client, operator_headers, sample_station_id):
        r = client.post("/api/sessions/acquire", headers=operator_headers,
    ...
    def test_heartbeat_session(self, client, operator_headers, sample_station_id):
        ...
        r = client.post("/api/sessions/heartbeat", headers=operator_headers,
    ...
    def test_release_session(self, client, operator_headers, sample_station_id):
        r = client.post("/api/sessions/acquire", headers=operator_headers,
                        params={"station_id": sample_station_id})
        r = client.post("/api/sessions/release", headers=operator_headers,
                        params={"station_id": sample_station_id})
        assert r.json()["status"] == "success"
...
class TestUserCRUD:
    ...
    def test_create_user(self, client, admin_headers):
        r = client.post("/api/users", headers=admin_headers, json={
            "username": "newuser", "password": "Pass123!", "role": "OPERATOR"
        })
        assert r.json()["status"] == "success"
    ...
    def test_create_user_invalid_role(self, client, admin_headers):
        r = client.post("/api/users", headers=admin_headers, json={
            "username": "badrole", "password": "Pass123!", "role": "SUPERUSER"
        })
        assert r.json()["status"] == "error"

    def test_update_user(self, client, admin_headers, operator_user_id):
        r = client.put(f"/api/users/{operator_user_id}", headers=admin_headers, json={
    ...
    def test_reset_password(self, client, admin_headers, operator_user_id):
        r = client.put(f"/api/users/{operator_user_id}/password", headers=admin_headers, json={
            "password": "NewPass123!"
        })
        assert r.json()["status"] == "success"
...
class TestAnalytics:
    def test_analytics_today_empty(self, client, admin_headers, sample_station_id):
        r = client.get("/api/analytics/today", headers=admin_headers,
                       params={"station_id": sample_station_id})
        assert r.status_code == 200
...
class TestLivePreview:
    def test_live_preview(self, client, admin_headers, sample_station_id):
        r = client.get("/api/live", headers=admin_headers,
                       params={"station_id": sample_station_id})
        assert r.status_code == 200

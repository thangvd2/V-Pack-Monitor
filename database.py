            ...
            for path in row[0].split(','):
                if os.path.exists(path):
                    try:
                        ...
                        print(f"Error removing {path}: {e}")
            cursor.execute(
                "DELETE FROM packing_video WHERE id = ?", (record_id,))
        conn.commit()
...
def get_stations():
    with sqlite3.connect(DB_FILE) as conn:
        ...
        return [{"id": r[0],
                 ...
                 "safety_code": r[4],
                 "camera_mode": r[5],
                 "camera_brand": r[6]} for r in rows]
...
def update_station(station_id, data):
    with sqlite3.connect(DB_FILE) as conn:
        ...
        cursor.execute('''
            ...
            WHERE id=?
        ''', (data['name'], data['ip_camera_1'], data.get('ip_camera_2', ''), data['safety_code'], data['camera_mode'], data.get('camera_brand', 'imou'), station_id))
        conn.commit()
...
def add_station(data):
    with sqlite3.connect(DB_FILE) as conn:
        ...
        cursor.execute('''
            INSERT INTO stations (name, ip_camera_1, ip_camera_2, safety_code, camera_mode, camera_brand)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (data['name'], data['ip_camera_1'], data.get('ip_camera_2', ''), data['safety_code'], data['camera_mode'], data.get('camera_brand', 'imou')))
        conn.commit()
